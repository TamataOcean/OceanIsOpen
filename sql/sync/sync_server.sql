...
( --sous-select : toutes les données + pk value
	SELECT *,(json_array_elements(s.sauv)->>pk) id
	FROM sync.sauv_data s
	WHERE replay = FALSE
) al,
(--trouve les données modifiées plusieurs fois
	SELECT distinct  pk, (json_array_elements(s.sauv)->>pk) id, integrateur i,COUNT(integrateur)
	FROM sync.sauv_data s
	WHERE replay = FALSE
	GROUP BY pk, id, i 
...
WHERE trouv.pk = al.pk AND trouv.id = al.id AND trouv.i = al.integrateur AND al.ts NOT IN 
	(SELECT d.ts FROM
		...
		SELECT pk,(json_array_elements(s.sauv)->>pk) id,  max(ts) ts, integrateur
		FROM sync.sauv_data s
		WHERE replay = FALSE
...
SELECT  boolean 'f' supprime_data,tbl,pk,(json_array_elements(sauv)->>pk) id,integrateur,ts,schema_bd,action1,sauv,replay,no_replay
FROM ( --liste toutes les données sans les multi edition utilisateur.
	SELECT (json_array_elements(sauv)->>pk) id,* 
	FROM sync.sauv_data
...
AND al.id IN (--liste les single id pouvant être inséré tout de suite, si changement du "having" possibilité de trouver les doublons d'edition.
		SELECT distinct (json_array_elements(s.sauv)->>pk) id
		FROM sync.sauv_data s
...
ORDER BY tbl, (json_array_elements(sauv)->>pk) ASC;
